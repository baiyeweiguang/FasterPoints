...
class FastestDet:
    def __init__(self):
        ...
        if opt.weight is not None:
            ...
            self.model = FasterPoints(self.cfg.category_num, self.cfg.num_keypoints, True).to(device)
            self.model.load_state_dict(torch.load(opt.weight), strict=False)
        else:
            self.model = FasterPoints(self.cfg.category_num, self.cfg.num_keypoints, False).to(device)

        # # 打印网络各层的张量维度
        # input = torch.randn(1, 3, 416, 416).to(device)
        # flops, params = profile(self.model, inputs=(input, ))
        # print("flops:%f, params:%f"%(flops, params))
        # profiler.start()
        # self.model(input)
        # profiler.stop()
    ...
    def train(self):
        ...
        for epoch in range(self.cfg.end_epoch + 1):
            ...
            for imgs, targets in pbar:
                ...
                # 打印相关训练信息
                info = "Epoch:%d LR:%f Wing:%f Obj:%f Cls:%f Total:%f" % (
