...
from module.faster_points import Detector
...
# print(output)
    ...
    for box in output[0]:
        ...
        obj_score = box[0]
        if obj_score > 0.6:
            category = LABEL_NAMES[int(box[1])]
            x1, y1 = int(box[2] * W), int(box[3] * H)
            # print(box[6], box[7])
            x2, y2 = int(box[4] * W), int(box[5] * H)
            x3, y3 = int(box[6] * W), int(box[7] * H)
            x4, y4 = int(box[8] * W), int(box[9] * H)
            
            # cv2.circle(ori_img, (x1, y1), 3, (0, 0, 255), -1)
            # cv2.circle(ori_img, (x2, y2), 3, (0, 0, 255), -1)
            # cv2.circle(ori_img, (x3, y3), 3, (0, 0, 255), -1)
            # cv2.circle(ori_img, (x4, y4), 3, (0, 0, 255), -1)
...
if __name__ == '__main__':
    ...
    model = Detector(cfg.category_num, 4, True).to(device)
